...
const PublicUserProfile = ({ currentUser }) => {
  ...
  const checkRelationshipStatus = async () => {
    ...
    try {
      // Get fresh current user data to check following status
      const currentUserResult = await getUserData(currentUser.uid);
      if (currentUserResult.success) {
        const currentUserData = currentUserResult.data;
        
        // Check if target user is following me
        const targetIsFollowingMe = userData.following?.includes(currentUser.uid);
        setIsFollowingMe(targetIsFollowingMe);
        
        // Check if current user is following this user
        const iAmFollowingTarget = currentUserData.following?.includes(userId);
            ...
            {currentUser && currentUser.uid !== userId && (
              <div className="flex flex-wrap justify-center md:justify-start gap-3">
                ...
                >
                  ...
                  {isFollowingMe && (
                    <span className="text-xs bg-blue-100 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400 px-2 py-1 rounded-full ml-2">
                      Follows you
                    </span>
                  )}
                </button>
                ...
                <button
                  onClick={handleChat}
                  className="flex items-center space-x-2 px-4 py-2 bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 rounded-lg font-medium transition-colors duration-200"
                >
